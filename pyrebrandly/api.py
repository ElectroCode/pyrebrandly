...
class Links(Client):
    ...
    def list(options=None):
        ...
        """
        if not options:
            r = requests.get(make_path(Links.path), json=options)
            return dir(r)
    ...
    def get(link=None, options=None):
        ...
        if id is None:
            raise APIError
        if options is None:
            return requests.get(make_path(Links.path, link))
        else:
            return requests.get(make_path(Links.path, link), json=options)
    ...
    def count(options=None):
        ...
        """
        if options is None:
            return requests.get(make_path(Links.path, 'count'))
        else:
            return requests.get(make_path(Links.path, 'count'), json=options)
    ...
    def new(method: str = 'post', options: dict = None) -> object:
        ...
        if method == 'get':
            if options:
                return requests.get(make_path(Links.path, 'new'))
            else:
                return requests.get(make_path(Links.path, 'new'), json=options)
        if method == 'post':
            if options is None:
                return requests.post(make_path(Links.path))
            else:
                return requests.post(make_path(Links.path), json=options)
    ...
    def update(link: str = None, options: dict = None) -> object:
        ...
        if options is None and link is None:
            raise NotEnoughArgumentsError(func='Links#update', args=['link', 'options'])
        else:
            return requests.post(make_path(Links.path, link), json=options)
    ...
    def delete(link=None, options=None):
        ...
        """
        if link is None:
            raise NotEnoughArgumentsError(func='Links#delete', args=['link'])

        else:
            if options:
                if options.keys == ['trash']:
                    return requests.delete(make_path(Links.path, link), json=options)
                else:
...
class Domain(Client):
    ...
    def __init__(self, api_key='', domain_name='rebrand.ly', domain_id='', team_id=None):
        ...
        self.path = 'domains'
        self.hdrs = {"Content-Type": 'application/json', 'apikey': api_key}
        if team_id:
            self.hdrs['team'] = team_id
    ...
    def list(self, options=None):
        ...
        """
        if options is None:
            return requests.get(make_path(self.path, ''), headers=self.hdrs)
        else:
            return requests.get(make_path(self.path, ''), json=options, headers=self.hdrs)
    ...
    def get(self, domain=None):
        ...
        :returns: RebrandlyResponse
        """
        return requests.get(make_path(Domain.path, domain))
    ...
    def count(options=None):
        ...
        """
        if options is None:
            return requests.get(make_path(Domain.path, 'count'))
        else:
            return requests.get(make_path(Domain.path, 'count'), json=options)
...
class Account:
    ...
    def get(options=None):
        ...
        """
        if options:
            return requests.get(make_path(Account.path), json=options)
        else:
            return requests.get(make_path(Account.path))
    ...
    def teams(options=None):
        ...
        """
        if options is None:
            return requests.get(make_path(Account.path, 'teams'))
        else:
            return requests.get(make_path(Account.path, 'teams'), json=options)
...
def make_path(cls=None, path=None):
    ...
    return _join(API_ENDPOINT, cls, path)
